...
- Concierge is the normal authenticated landing. Use the shared white/stone/moss system, a readable transcript, the current person, and a prominent composer. Desktop may place resumable history beside the conversation. On mobile, history starts collapsed and the transcript scrolls within the available space so the composer and Send action remain reachable. Preserve ordinary page scrolling when enlarged text or a small keyboard viewport needs more room.
- Conversation receipts show verified saved results, useful source links, and explicit review controls. Keep model payloads, provider configuration, and internal execution state out of everyday copy. Streaming announcements describe progress once; interrupted requests retain completed receipts and offer recovery. Sensitive context selection applies to the individual request.
