  ...
  class GoogleOauth
    ...
    class << self
      ...
      def authorization_url(state:, redirect_uri:)
        raise ConnectionError.new(code: "provider_unavailable") unless available?
      ...
      def token_request(existing_refresh_token: nil, **parameters)
        ...
        raise ConnectionError.new(code: transient_code) if transient_code

        payload = JSON.parse(response.body)
        unless response.is_a?(Net::HTTPSuccess)
          raise ConnectionError.new(code: token_error_code(response, payload))
        end

        scopes = payload.fetch("scope", CalendarConnection::GOOGLE_SCOPE).to_s.split
        ...
        unless payload["access_token"].present? && refresh_token.present? && scopes.include?(CalendarConnection::GOOGLE_SCOPE)
          raise ConnectionError.new(code: "calendar_authorization_incomplete", credentials: partial_credentials)
        end
        ...
        raise ConnectionError.new(code: "invalid_provider_response", credentials: partial_credentials)
      rescue IOError, SocketError, SystemCallError, Timeout::Error, OpenSSL::SSL::SSLError,
        Net::HTTPBadResponse, Net::HTTPHeaderSyntaxError
        raise ConnectionError.new(code: "provider_unavailable")
      end
      ...
      def client_id = ENV["GOOGLE_CALENDAR_CLIENT_ID"]
      def client_secret = ENV["GOOGLE_CALENDAR_CLIENT_SECRET"]
    end
  end
end
