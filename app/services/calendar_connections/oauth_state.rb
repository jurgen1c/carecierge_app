module CalendarConnections
  class OauthState
    SESSION_KEY = :calendar_oauth_nonce
    MAX_AGE = 10.minutes
    ...
    def self.issue(user:, session:)
      ...
      verifier.generate(
        { user_id: user.id, nonce:, connection_generation: user.calendar_connection_generation },
        expires_in: MAX_AGE
      )
    end
    ...
    def self.verifier
      Rails.application.message_verifier("calendar-connection-oauth-state")
    end
