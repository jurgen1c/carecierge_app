...
module Contacts
  class GoogleOauth
    ...
    class << self
      def available?
        client_id.present? && client_secret.present? && ENV["GOOGLE_CONTACTS_ISOLATED_PROJECT"] == "true"
      end
      ...
      def token_request(existing_refresh_token: nil, **parameters)
        ...
        scopes = payload.fetch("scope", existing_refresh_token ? ContactsConnection::GOOGLE_SCOPE : "").to_s.split
        ...
        unless payload["access_token"].present? && refresh_token.present? && scopes.include?(ContactsConnection::GOOGLE_SCOPE)
      ...
      def client_id = ENV["GOOGLE_CONTACTS_CLIENT_ID"]
      def client_secret = ENV["GOOGLE_CONTACTS_CLIENT_SECRET"]
    end
  end
end
