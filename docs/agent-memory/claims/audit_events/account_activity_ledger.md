...
claim: >
  ...
  connected status can commit; duplicate, unsupported, or reordered settings
  inputs are normalized away and emit no event when the supported selection set
  is unchanged. Lease renewal itself emits no audit event, and disconnect flushes
  any count retained after a partial failure before removing the connection. A sync
