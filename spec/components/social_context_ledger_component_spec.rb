...
RSpec.describe SocialContextLedgerComponent, type: :component do
  it "renders the approved compact ledger with an inline Lexxy composer and explicit trust controls" do
    ...
    expect(page).to have_css(
      "lexxy-editor[data-direct-upload-url='#{Rails.application.routes.url_helpers.social_context_direct_uploads_path}']",
      count: 2,
      visible: :all
    )
    expect(page).to have_css(
      "lexxy-editor[data-blob-url-template*='/social_context/screenshots/:signed_id/:filename']",
      count: 2,
      visible: :all
    )
  ...
  it "renders saved screenshots through the owner-authorized route" do
    profile = create(:relationship_profile)
    blob = create_social_context_image_blob(user: profile.user, filename: "private-context.png")
    note = create(
      :social_context_note,
      relationship_profile: profile,
      body: "<p>Context</p>#{ActionText::Attachment.from_attachable(blob).to_html}"
    )
    permission = AutomationPermissionDecision.new(
      capability: AutomationCapability.fetch("analyze_uploaded_social_content"),
      mode: "disabled"
    ...
    editor = page.find("lexxy-editor[id='social_context_note_#{note.id}_body']", visible: :all)
    attachment = Nokogiri::HTML.fragment(editor[:value]).at_css("action-text-attachment")
    preview = Nokogiri::HTML.fragment(JSON.parse(attachment["content"])).at_css("img")
    expect(preview["src"]).to include("/social_context/screenshots/")
    expect(preview["loading"]).to eq("lazy")
    expect(preview["decoding"]).to eq("async")
    expect(rendered_content).not_to include("/rails/active_storage/blobs/")
  end
