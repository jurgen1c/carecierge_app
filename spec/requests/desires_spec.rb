...
RSpec.describe "Desires", type: :request do
  ...
  describe "GET /relationship_profiles/:relationship_profile_id" do
    ...
    it "renders localized desire copy in Spanish" do
      ...
      expect(response.body).not_to include("desire_fulfillment[fulfilled_on]")
  ...
  describe "POST /relationship_profiles/:relationship_profile_id/desires" do
    ...
    it "ignores forged terminal statuses on generic create params" do
      ...
      expect(response.media_type).to eq("text/vnd.turbo-stream.html")
      expect(profile.desires.sole).to have_attributes(status: "active", source: "manual")
    end
  end
  ...
  describe "PATCH /relationship_profiles/:relationship_profile_id/desires/:id/fulfill" do
    ...
    it "uses the server date when no fulfillment date is submitted" do
      user = create(:user)
      profile = create(:relationship_profile, user:)
      desire = create(:desire, relationship_profile: profile, title: "Fresh flowers")
      sign_in user

      travel_to Time.zone.local(2026, 7, 8, 9, 0, 0) do
        expect do
          patch fulfill_relationship_profile_desire_path(profile, desire), as: :turbo_stream
        end.to change(DesireFulfillment, :count).by(1)
      end

      expect(desire.fulfillments.sole.fulfilled_on).to eq(Date.new(2026, 7, 8))
    end
  end
