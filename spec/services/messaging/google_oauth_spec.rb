...
RSpec.describe Messaging::GoogleOauth do
  before do
    allow(ENV).to receive(:[]).and_call_original
    allow(ENV).to receive(:[]).with("GOOGLE_MESSAGING_ISOLATED_PROJECT").and_return("true")
    allow(ENV).to receive(:[]).with("GOOGLE_MESSAGING_CLIENT_ID").and_return("messaging-client")
    allow(ENV).to receive(:[]).with("GOOGLE_MESSAGING_CLIENT_SECRET").and_return("messaging-secret")
  end

  it "builds an offline authorization URL with the read-only messaging scope" do
  ...
  it "is unavailable without dedicated messaging credentials" do
    allow(ENV).to receive(:[]).with("GOOGLE_MESSAGING_CLIENT_ID").and_return(nil)
    allow(ENV).to receive(:[]).with("GOOGLE_MESSAGING_CLIENT_SECRET").and_return(nil)
  ...
  it "exchanges a code for normalized credentials without exposing client secrets" do
    ...
    expect(credentials).to have_attributes(
      access_token: "private-access",
      refresh_token: "private-refresh",
      scopes: [ MessagingConnection::GOOGLE_SCOPE ]
    )
    expect(Rack::Utils.parse_query(request.body)).to include(
      "code" => "oauth-code",
      "client_id" => "messaging-client",
      "client_secret" => "messaging-secret",
  ...
  it "retains the existing refresh token when Google rotates only the access token" do
    ...
    expect(credentials.scopes).to eq([ MessagingConnection::GOOGLE_SCOPE ])
  end
  ...
  it "preserves live tokens from an incomplete exchange for revocation" do
    ...
    expect { described_class.exchange(code: "oauth-code", redirect_uri: "https://example.test/callback") }
      .to raise_error(Messaging::Error) { |error|
  ...
  it "revokes newly exchanged credentials without persisting or exposing them" do
    ...
    credentials = Messaging::GoogleOauth::Credentials.new(
      access_token: "private-access",
      refresh_token: "private-refresh",
  ...
  it "normalizes malformed and provider error responses into safe codes" do
    ...
    expect { described_class.refresh(refresh_token: "existing-refresh") }
      .to raise_error(Messaging::Error) { |error| expect(error.code).to eq("invalid_provider_response") }
  end
  ...
  it "normalizes DNS failures into a recoverable safe code" do
    ...
    expect { described_class.refresh(refresh_token: "existing-refresh") }
      .to raise_error(Messaging::Error) { |error| expect(error.code).to eq("provider_unavailable") }
  end
  ...
  it "normalizes token endpoint throttling and outages into retryable safe codes" do
    ...
    expect { described_class.refresh(refresh_token: "existing-refresh") }
      .to raise_error(Messaging::Error) { |error| expect(error.code).to eq("rate_limited") }
    expect { described_class.refresh(refresh_token: "existing-refresh") }
      .to raise_error(Messaging::Error) { |error| expect(error.code).to eq("provider_unavailable") }
  end

  it "rejects unexpected write scopes and retains credentials for cleanup" do
    http = instance_double(Net::HTTP)
    allow(Net::HTTP).to receive(:start).and_yield(http)
    allow(http).to receive(:request).and_return(response(Net::HTTPOK, body: {
      access_token: "access", refresh_token: "refresh", scope: "#{MessagingConnection::GOOGLE_SCOPE} https://www.googleapis.com/auth/gmail.send"
    }.to_json))
    expect { described_class.exchange(code: "code", redirect_uri: "https://example.test/callback") }
      .to raise_error(Messaging::Error) { |error| expect(error.credentials.access_token).to eq("access") }
  end

  it "rejects non-object token responses as a recoverable error" do
    http = instance_double(Net::HTTP)
    allow(Net::HTTP).to receive(:start).and_yield(http)
    allow(http).to receive(:request).and_return(response(Net::HTTPOK, body: "null"))
    expect { described_class.exchange(code: "code", redirect_uri: "https://example.test/callback") }.to raise_error(Messaging::Error)
  end
